...
describe('POMODORO_CONSTANTS 自動開始機能関連定数テスト', () => {
  ...
  describe('NOTIFICATION_PERMISSION_REQUEST_DELAY', () => {
    test('通知権限要求遅延時間が適切な値に設定されている', () => {
      expect(POMODORO_CONSTANTS.NOTIFICATION_PERMISSION_REQUEST_DELAY).toBe(1000)
      expect(typeof POMODORO_CONSTANTS.NOTIFICATION_PERMISSION_REQUEST_DELAY).toBe('number')
      expect(POMODORO_CONSTANTS.NOTIFICATION_PERMISSION_REQUEST_DELAY).toBeGreaterThan(0)
    })
  })
